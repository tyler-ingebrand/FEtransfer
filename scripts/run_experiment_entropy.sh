...
run_experiment() {
  ...
  LOGDIR="text_logs/entropy/$DATA/$ALGO/$SEED"
  mkdir -p $LOGDIR
  LOGFILE="$LOGDIR/log.txt"

  echo "Running CROSS ENTROPY experiment #$COUNT/$total_count: Dataset=$DATA, Alg=$ALGO, Seed=$SEED on GPU $GPU"

  # TODO: Replace the following line with your actual experiment command
  # TODO: Ensure your command uses the specified GPU
  python test.py --alg $ALGO --dataset $DATA --seed $SEED --device $GPU --epochs $EPOCHS --log_dir logs/entropy --cross_entropy > $LOGFILE 2>&1
