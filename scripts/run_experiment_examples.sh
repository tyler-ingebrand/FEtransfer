...
run_experiment() {
  ...
  LOGDIR="text_logs/examples/$DATA/$ALGO/$SEED"
  mkdir -p $LOGDIR
  LOGFILE="$LOGDIR/log.txt"

  echo "Running experiment #$COUNT/$total_count: Dataset=$DATA, Alg=$ALGO, N_examples=$N_EX, Seed=$SEED on GPU $GPU"

  # TODO: Replace the following line with your actual experiment command
  # TODO: Ensure your command uses the specified GPU
  python test.py --alg $ALGO --dataset $DATA --seed $SEED --device $GPU --epochs $EPOCHS --log_dir logs/examples --n_examples $N_EX > $LOGFILE 2>&1
    ...
    while true; do
      for ((i=0; i<$NUM_GPUS; i++)); do
        ...
        if flock $LOCK_FILE bash -c "[ \$(< $STATUS_DIR/gpu_$i) -lt $PROCESSES_PER_GPU ]"; then
          ...
          run_experiment $DATA $ALGO $SEED $i $COUNT $N_EXAMPLES &
          break 2
        fi
      done
...
for dataset in $DATASETS; do
   for alg in $ALGS; do
     for n_ex in $N_EXAMPLES; do
       for seed in {1..3}; do
         job_list+=("$dataset $alg $seed $total_count $n_ex")
