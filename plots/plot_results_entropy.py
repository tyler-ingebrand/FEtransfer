...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(logdir, dataset)):
        ...
        print("If you think this is an error, run collect_tensorboards_entropy.py first.")
        exit()
    ...
    # plots below axs
    leg = fig.legend(handles, labels, loc="lower center", ncol=len(algs)//2, fontsize=FONT_SIZE,frameon=False, bbox_to_anchor=(0.5, -0.03))
    # make the legend lines thicker
    for h in leg.get_lines():
        h.set_linewidth(5)
    
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.255)  # Increase bottom margin slightly for space
